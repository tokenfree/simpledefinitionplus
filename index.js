...
function loadSavedTheme() {
    ...
    if (savedTheme === 'dark') {
        ...
        document.getElementById('themeBtn').title = 'Switch to Light Mode';
    } else {
        isDarkTheme = false;
        document.documentElement.removeAttribute('data-theme');
...
function loadSavedDefinitions() {
    ...
    if (savedDefinitions) {
        try {
            ...
            console.error('Error loading saved definitions:', e);
            definitions = [];
        }
    }
}
...
// Update addDefinition function to save
function addDefinition() {
    const number = parseInt(matterInput.value);
    const text = quill.root.innerHTML.trim();
...
// Update parseAndAddDefinitions function to save
function parseAndAddDefinitions() {
    const text = quill.getText().trim();
...
// Update deleteCurrentDefinition function to save
function deleteCurrentDefinition() {
    if (definitions.length === 0) {
        alert('No definitions to delete');
        return;
...
// Update clearAllDefinitions function to save
function clearAllDefinitions() {
    if (definitions.length === 0) {
        alert('No definitions to clear');
        return;
